...
// ─── Main component ───────────────────────────────────────────────────────────
export default function QRCodeGenerator({ onNavigateToBilling, onSaved, planId = 'free' }) {
        ...
        {step === 0 && (
          <motion.div key="step0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
            ...
            <Field label={<>Story <span className="text-red-400">*</span></>} error={errors.story}>
              ...
              <div
                data-guide="story-wrap"
                data-filled={stripHtml(form.story).length > 0 ? 'true' : 'false'}
                className={`sf-story-quill${errors.story ? ' sf-story-quill--err' : ''}`}
              >
                <ReactQuill
                  theme="snow"
                  value={form.story}
